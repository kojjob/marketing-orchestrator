defmodule GrowthOsWeb.UserAuthTest do
  use GrowthOsWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias GrowthOs.Tenancy
  alias GrowthOs.Tenancy.Scope
  alias GrowthOsWeb.UserAuth

  import GrowthOs.TenancyFixtures

  @remember_me_cookie "_growth_os_web_user_remember_me"
  @remember_me_cookie_max_age 60 * 60 * 24 * 14

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, GrowthOsWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_user/3" do
    test "stores the user token in the session", %{conn: conn, user: user} do
      ...
      assert Tenancy.get_user_by_session_token(token)
    end
  ...
  describe "logout_user/1" do
    test "erases session and cookies", %{conn: conn, user: user} do
      user_token = Tenancy.generate_user_session_token(user)
      ...
      assert redirected_to(conn) == ~p"/"
      refute Tenancy.get_user_by_session_token(user_token)
    end

    test "broadcasts to the given live_socket_id", %{conn: conn} do
      live_socket_id = "users_sessions:abcdef-token"
      GrowthOsWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "fetch_current_scope_for_user/2" do
    ...
    test "reissues a new token after a few days and refreshes cookie", %{conn: conn, user: user} do
      ...
      {user, _} = Tenancy.get_user_by_session_token(token)
  ...
  describe "on_mount :require_sudo_mode" do
    ...
    test "redirects when authentication is too old", %{conn: conn, user: user} do
      ...
      user_token = Tenancy.generate_user_session_token(user)
      {user, token_inserted_at} = Tenancy.get_user_by_session_token(user_token)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        GrowthOsWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
